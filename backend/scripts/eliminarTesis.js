...
if (!tesisId) {
    ...
    console.log('Uso: node scripts/eliminarTesis.js <ID>');
    process.exit(1);
}
...
db.serialize(() => {
    // Verificar si existe
    db.get('SELECT * FROM tesis WHERE id = ?', [id], (err, tesis) => {
        if (err) {
        ...
        // Eliminar
        db.run('DELETE FROM tesis WHERE id = ?', [id], function (err) {
            if (err) {
            ...
            db.all('SELECT id, titulo, autor FROM tesis ORDER BY id', [], (err, todasTesis) => {
                if (err) {
                    console.error('❌ Error:', err);
                } else {
